...
public sealed partial class Editor : Form
{
    ...
    {
        if (PlayerIndex < 0 || PlayerIndex >= SAV.Players.Count)
            return;

        const decimal walletMax = 99_999m;
        const decimal bankMax = 999_999_999m;
        const decimal nookMilesMax = 9_999_999m;
        const decimal earnedMilesMax = 999_999_999m;
        const decimal pokiMax = 999_999_999m;
        const decimal hotelTicketsMax = 999_999_999m;
        ...
        if (NUD_HotelTickets.Visible)
            NUD_HotelTickets.Value = Math.Min(NUD_HotelTickets.Maximum, hotelTicketsMax);
    }
    ...
    private void LoadPlayer(int index)
    {
        ...
        {
            L_HotelTickets.Visible = NUD_HotelTickets.Visible = true;
