...
public sealed class HorizonSave(ISaveFileProvider provider)
{
    ...
    public static HorizonSave FromFolder(string folder)
    {
        ...
        var provider = new FolderSaveFileProvider(primary, mirrors);
        return new HorizonSave(provider);
    }

    /// <summary>
    /// Resolves the primary save folder and any sibling folders that should mirror writes.
    /// Supports users who keep both "0000000000000000" and "0000000000000001" folders in sync
    /// by pointing at either an individual save folder or the parent wrapper.
    /// </summary>
    private static (string Primary, string[] Mirrors) ResolveSaveFolders(string folder)
    {
        var normalized = Path.GetFullPath(folder);

        if (File.Exists(Path.Combine(normalized, "main.dat")) && File.Exists(Path.Combine(normalized, "mainHeader.dat")))
        {
            var parent = Directory.GetParent(normalized);
            if (parent is null)
                return (normalized, []);

            var mirrors = Directory.GetDirectories(parent.FullName, "*", SearchOption.TopDirectoryOnly)
                .Where(d => !PathsEqual(d, normalized) && File.Exists(Path.Combine(d, "main.dat")) && File.Exists(Path.Combine(d, "mainHeader.dat")))
                .Select(Path.GetFullPath)
                .ToArray();
            return (normalized, mirrors);
        }

        var candidates = Directory.GetDirectories(normalized, "*", SearchOption.TopDirectoryOnly)
            .Where(d => File.Exists(Path.Combine(d, "main.dat")) && File.Exists(Path.Combine(d, "mainHeader.dat")))
            .Select(d => (Path: Path.GetFullPath(d), Timestamp: File.GetLastWriteTimeUtc(Path.Combine(d, "main.dat"))))
        ...
        var mirrorSet = candidates.Skip(1).Select(x => x.Path).ToArray();
        return (primary, mirrorSet);
    }
